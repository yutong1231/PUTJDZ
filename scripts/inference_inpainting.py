...
class ImagePathDataset(torch.utils.data.Dataset):
    def __init__(self, image_dir, mask_dir=None, size=(256,256)):
        if os.path.isfile(image_dir):
            ...
            image_paths = sorted(get_all_file(image_dir, end_with=['.png','.jpg','jpeg'],path_type='relative'))
        ...
        if mask_dir is not None:
            if os.path.isfile(mask_dir):
                ...
                mask_paths = sorted(get_all_file(mask_dir, end_with=['.png','.jpg','jpeg'],path_type='relative'))
        else:
            mask_paths = None
